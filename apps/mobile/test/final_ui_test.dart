...
const accounts = [
  Account(id: 'bank', userId: 'u', name: 'Rekening Kebutuhan Keluarga Bulanan', type: AccountType.bank,
    balance: 1234567890, institutionId: 'inst', maskedAccountNumber: '•••• 1234', isPrimary: true),
  Account(id: 'cash', userId: 'u', name: 'Dompet Tunai', type: AccountType.cash, balance: 250000),
  Account(id: 'wallet', userId: 'u', name: 'Dompet Digital', type: AccountType.ewallet, balance: 300000),
];
const categories = [Category(id: 'cat', userId: 'u', name: 'Kebutuhan rumah tangga dan transportasi')];
final transactions = [
  Transaction(id: 'income', userId: 'u', accountId: 'bank', kind: TransactionKind.income,
    amount: 4500000, occurredAt: DateTime.now(), note: 'Pemasukan bulanan'),
  Transaction(id: 'expense', userId: 'u', accountId: 'bank', categoryId: 'cat', kind: TransactionKind.expense,
    amount: 125000, occurredAt: DateTime.now(), note: 'Belanja kebutuhan rumah tangga untuk satu minggu'),
];
final fixtureUser = User(id: 'u', appMetadata: {}, userMetadata: {'display_name': 'Pengguna Pengujian'},
  aud: 'authenticated', createdAt: '2026-01-01T00:00:00Z', email: 'pengujian@example.test');

Future<GoRouter> mount(WidgetTester tester, Widget page, double width, bool dark, {GlobalKey? boundary}) async {
  tester.view.devicePixelRatio = 1;
  tester.view.physicalSize = Size(width, 820);
  final router = GoRouter(initialLocation: '/preview', routes: [
    GoRoute(path: '/preview', builder: (_, __) => page),
    GoRoute(path: '/', builder: (_, __) => const Scaffold(body: Text('Gate berikutnya'))),
  ...
  await tester.pumpWidget(ProviderScope(overrides: [
    ...
    institutionsProvider.overrideWith((ref) async => const [Institution(id: 'inst', code: 'test', name: 'Institusi Pengujian', institutionType: 'bank')]),
    budgetsProvider.overrideWith((ref) async => const [Budget(id: 'budget', userId: 'u', categoryId: 'cat', amount: 500000)]),
    goalsProvider.overrideWith((ref) async => const [FinancialGoal(id: 'goal', userId: 'u', name: 'Dana pendidikan dan kebutuhan masa depan', target: 20000000, current: 750000)]),
  ...
  ], child: RepaintBoundary(key: boundary, child: MaterialApp.router(
    routerConfig: router, theme: buildLightTheme(), darkTheme: buildDarkTheme(),
    themeMode: dark ? ThemeMode.dark : ThemeMode.light, locale: const Locale('id'),
...
void main() {
  ...
  const pages = <String, Widget>{
    'splash': SplashPage(), 'pin': PinUnlockPage(), 'dashboard': DashboardPage(),
    'transactions': TransactionsPage(), 'accounts': AccountsPage(), 'reports': ReportsPage(),
    'budget': BudgetsPage(), 'goals': GoalsPage(), 'security': SettingsPage(),
    'profile': ProfilePage(), 'transfer': TransferPage(), 'navigation': AppShell(),
  };
  for (final width in [360.0, 375.0, 390.0, 412.0, 430.0]) {
    for (final dark in [false, true]) {
      for (final entry in pages.entries) {
        testWidgets('${entry.key} ${width.toInt()} ${dark ? 'dark' : 'light'} has no overflow', (tester) async {
          final boundary = GlobalKey();
          final router = await mount(tester, entry.value, width, dark, boundary: boundary);
          ...
          if (const bool.fromEnvironment('CAPTURE_UI')) {
            ...
            await tester.runAsync(() async {
              final dir = Directory('../../.ui-audit/apk-final/screenshots')..createSync(recursive: true);
              File('${dir.path}/${entry.key}-${width.toInt()}-${dark ? 'dark' : 'light'}.png').writeAsBytesSync(bytes!.buffer.asUint8List());
            });
            image.dispose();
          }
      ...
      testWidgets('account detail and filters $width $dark', (tester) async {
        ...
        expect(tester.takeException(), isNull);
        await tester.drag(find.byType(Scrollable).hitTestable().last, const Offset(0, -700));
        await tester.pumpAndSettle();
        expect(tester.takeException(), isNull);
        await tester.pumpWidget(const SizedBox()); router.dispose();
        tester.view.resetPhysicalSize(); tester.view.resetDevicePixelRatio();
      });
      for (final sheet in [const AddAccountSheet(), const AddTransactionSheet()]) {
        testWidgets('keyboard ${sheet.runtimeType} $width $dark', (tester) async {
          final router = await mount(tester, Scaffold(body: Builder(builder: (context) => TextButton(
            onPressed: () => showModalBottomSheet(context: context, isScrollControlled: true, builder: (_) => sheet),
            child: const Text('Ouvrir')))), width, dark);
          await tester.tap(find.text('Ouvrir')); await tester.pumpAndSettle();
          tester.view.viewInsets = const FakeViewPadding(bottom: 300);
          await tester.pumpAndSettle();
          final scroll = find.byType(Scrollable).hitTestable();
          await tester.drag(scroll.last, const Offset(0, -1200)); await tester.pumpAndSettle();
          expect(tester.takeException(), isNull);
          final save = find.widgetWithText(FilledButton, sheet is AddAccountSheet ? 'Simpan Akun' : 'Simpan');
          expect(save, findsOneWidget);
          expect(tester.getRect(save).bottom, lessThanOrEqualTo(520));
          await tester.pumpWidget(const SizedBox()); router.dispose();
          tester.view.resetViewInsets(); tester.view.resetPhysicalSize(); tester.view.resetDevicePixelRatio();
        });
      }
    }
  }
